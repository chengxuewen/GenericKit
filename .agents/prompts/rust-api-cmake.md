...
gkit_media/
  ...
  examples/
    gkit-media-viewer, gkit-media-viewer_build, gkit-media-viewer_run,
    gkit-media-square-gen, gkit-media-square-gen_build, gkit-media-square-gen_run,
    gkit_examples_assets
...
| `gkit_cargo_add_example` | `GKitCargoExample.cmake` | 为 Rust example 创建 _build / _run / aggregate 三段 CMake target |
...
### gkit-media-square-gen

`crates/gkit-media/examples/gkit-media-square-gen/`：
- egui 应用：Start/Stop 按钮控制 VideoFrameGenerator 产帧，实时显示彩色方块动画
- VideoFrameGenerator → I420 → RGBA → egui texture 管线
- 无 assets 依赖，纯 test pattern 生成
- CMake target：`gkit-media-square-gen`, `_build`, `_run`
- FOLDER：`gkit_media/examples`
- 使用 `gkit_cargo_add_example()` 宏创建（见下方）

### GKitCargoExample.cmake

`cmake/GKitCargoExample.cmake` 提供 `gkit_cargo_add_example()` 函数，抽象出可复用的 Rust example CMake target 模式：

```cmake
gkit_cargo_add_example(
    NAME <name>              # example 名称（即 cargo --example 值）
    CRATE <crate>            # Rust crate 名称
    [ASSETS <dir>]           # 可选：assets 源目录
    [FOLDER <folder>]        # IDE FOLDER 属性
    [RELEASE]                # 使用 --release 编译
    [REQUIRED_CARGO]         # find_program cargo REQUIRED（默认 ON）
)
```

创建三阶段 target：
- `<name>_build`：`cargo build -p <crate> --example <name> [--release]`，POST_BUILD 复制二进制到 `build/examples/`
- `<name>_run`：依赖 `_build`（+ assets），从 `build/examples/` 执行
- `<name>` (ALL)：aggregate，依赖 `_build`（+ assets）
- `gkit_examples_<name>_assets`：若指定 ASSETS，复制到 `build/assets/<name>/`

典型用法（无 assets）：
```cmake
include(GKitCargoExample)
gkit_cargo_add_example(NAME gkit-media-square-gen CRATE gkit-media FOLDER gkit_media/examples)
```

根 CMakeLists.txt 中统一添加（受 `GKIT_BUILD_EXAMPLES` 守卫）：
```cmake
