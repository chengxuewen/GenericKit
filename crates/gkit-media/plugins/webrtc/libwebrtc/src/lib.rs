...
pub extern "C" fn gkit_plugin_destroy_factory(ptr: *mut c_void) {
    if !ptr.is_null() {
        unsafe { let _ = Box::from_raw(ptr as *mut Box<dyn PeerConnectionFactory>); }
    }
}

#[unsafe(no_mangle)]
pub extern "C" fn gkit_plugin_factory_create_pc(factory_ptr: *mut c_void) -> *mut c_void {
    if factory_ptr.is_null() { return std::ptr::null_mut(); }
    unsafe {
        let f: &Box<dyn PeerConnectionFactory> = &*(factory_ptr as *const Box<dyn PeerConnectionFactory>);
...
pub extern "C" fn gkit_plugin_destroy_pc(ptr: *mut c_void) {
    if !ptr.is_null() {
        unsafe { let _ = Box::from_raw(ptr as *mut Box<dyn gkit_media::protocols::rtc::client::core::PeerConnection>); }
    }
}
